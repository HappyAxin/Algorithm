...
接下来，如果还有剩余的k次操作且k为奇数，需要找到数组中的最小值，并将其取反。
...
收集每天的正利润就可以，**收集正利润的区间，就是股票买卖的区间，而只需要关注最终利润，不需要记录区间**。
...
然后，从左到右遍历数组，如果右边孩子的评分更高，给右边孩子分配的糖果数为左边孩子糖果数加1。

接下来，从右到左遍历数组，处理左边评分更高的孩子获得更多糖果的情况。如果左边孩子的评分更高，取当前孩子糖果数和右边孩子糖果数加1的较大值。

最后，计算总的糖果数，即每个孩子分配的糖果数的总和。
...
然后，如果当前位置达到了当前能够到达的最远位置`currEnd`，说明需要进行一次跳跃。增加跳跃次数`jumps`，并将`currEnd`更新为`currFarthest`，表示下一次跳跃的范围。
...
如果当前剩余汽油量小于0，表示无法从当前加油站前往下一个加油站。此时，需要将起始加油站设置为下一个加油站，并将当前剩余汽油量重置为0。

最后，判断总的剩余汽油量是否大于等于0，如果是，则表示可以绕环路行驶一周，返回起始加油站的编号；否则，返回-1。
...
- 使用一个数组 `dp` 来保存拆分整数 i 可获得的最大乘积。
- 初始状态：拆分整数 1 的最大乘积为 1。
- 接下来，从整数 2 开始，逐步计算拆分整数 i 的最大乘积。
- 对于整数 i，可以将其拆分为 j 和 i-j 两部分，其中 j 的范围是 1 到 i-1。需要比较三种情况的乘积大小：j * (i - j)、j * dp[i - j]、dp[i]（不拆分整数 i）。
...
- 使用一个数组 `dp` 来保存节点数为 i 的二叉搜索树的种数。
- 初始状态：节点数为 0 和 1 的二叉搜索树的种数都为 1。
- 接下来，从节点数为 2 开始，逐步计算节点数为 i 的二叉搜索树的种数。
- 对于节点数为 i 的二叉搜索树，可以选择任意一个节点作为根节点，然后将剩余的节点分为左子树和右子树。
- 需要遍历所有可能的根节点，将左子树和右子树的种数相乘，累加到总种数上。
...
- 在两个嵌套的循环中，需要计算每个子问题的结果，每个子问题的计算都需要常数时间。
- 因此，**时间复杂度**为 O($n * target$)，其中 n 是数组的长度，target 是数组元素和的一半。

**空间复杂度分析**

- 使用了一个二维数组 `dp` 来保存前 i 个数能否组成和为 j，数组的大小为 $(n+1) * (target+1)$。
...
- 首先，计算石头的总重量 sum。
- 的目标是选出一些石头，使得剩下的石头重量尽可能接近 sum / 2。
...
- 使用了一个一维数组 `dp` 来保存是否能够得到重量为 j 的石头，数组的大小为 target+1。
- 因此，空间复杂度为 O($sum / $2)。
...
- 对于存在负数的情况，将问题转化为在数组中选择一些元素，使其和等于$ (sum + target) / 2$。
...
- 使用了一个一维数组 `dp` 来保存凑成和为 i 的方式数，数组的大小为 newTarget+1。
...
- 使用了一个一维数组 `dp` 来保存硬币组合数，数组的大小为 amount+1。
- 因此，空间复杂度为 O($amount$)。
...
- 使用了一个一维数组 `dp` 来保存元素组合个数，数组的大小为 target+1。
- 因此，空间复杂度为 O($target$)。
...
- 使用了一个一维数组 `dp` 来保存楼梯的方法数，数组的大小为 n+1。
- 因此，空间复杂度为 O($n$)。
...
- 使用了一个一维数组 `dp` 来保存凑成金额所需的最少硬币个数，数组的大小为 amount+1。
- 因此，空间复杂度为 O($amount$)。
...
- 使用了一个一维数组 `dp` 来保存凑成数字的完全平方数的最少数量，数组的大小为 n+1。
- 因此，空间复杂度为 O($n$)。
...
- 使用了一个布尔型数组 `dp` 来保存前 `i` 个字符是否可以拆分成字典中的单词，数组的大小为 n+1。
- 另外，使用了一个哈希集合 `wordSet` 来存储字典中的单词，其大小取决于字典的单词数量和平均长度。
...
[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/robbery/Solution198.java)

**代码说明**

- 使用一个数组 `dp` 存储每个房屋偷窃的最大金额。其中 `dp[i]` 表示偷窃前 `i` 个房屋能够得到的最大金额。
- 初始化数组 `dp` 的前两个元素，`dp[0]` 为第一个房屋的金额，`dp[1]` 为前两个房屋中金额较大的那个。
- 从第三个房屋开始，通过递推关系 `dp[i] = max(dp[i - 1], dp[i - 2] + nums[i])` 计算每个房屋能够得到的最大金额。
- 最终，`dp[n - 1]` 即为一夜之内能够偷窃到的最高金额。

**时间复杂度分析**

- 遍历数组 `nums`，时间复杂度为 O($n$)，其中 `n` 是房屋的数量。

**空间复杂度分析**

- 使用了一个数组 `dp`，空间复杂度为 O($n$)。

### 213.打家劫舍II

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/robbery/Solution213.java)

**代码说明**

- 在环形街道中，问题分为两种情况：一种是偷窃第1间房屋到第n-1间房屋（不偷窃最后一间房屋），另一种是偷窃第2间房屋到第n间房屋（不偷窃第一间房屋）。
- 分别计算这两种情况下能够偷窃到的最大金额，最后取两种情况下的最大金额作为答案。
- 为了计算某个范围内房屋的最大金额，使用动态规划算法，与之前的房屋偷窃问题相同。
- 在计算最大金额时，需要注意数组下标的范围变化，以适应环形街道的特殊情况。

**时间复杂度分析**

- 对每种情况下的房屋进行一次动态规划计算，时间复杂度为 O($n$)，其中 n 是房屋的数量。
...
[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/robbery/Solution337.java)

**代码说明**

- 使用递归的方式处理二叉树的节点，对于每个节点，计算偷窃该节点和不偷窃该节点的两种情况下的最大金额。
- 通过递归处理其左右子节点，并根据子节点的结果计算当前节点的最大金额。
- 最终返回偷窃根节点和不偷窃根节点两种情况下的最大金额中的较大值。

**时间复杂度分析**

- 递归遍历每个节点，时间复杂度为 O($n$)，其中 n 是二叉树中节点的数量。

**空间复杂度分析**

- 使用了递归调用的栈空间，最坏情况下，栈的深度等于二叉树的高度，空间复杂度为 O($h$)，其中 h 是二叉树的高度。

## 股票问题

### 121. 买卖股票的最佳时机

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/stock/Solution121.java)

**代码说明**

1. 首先判断数组的长度n，如果长度小于2，则无法进行买卖交易，最大利润为0。
2. 创建一个长度为n的动态规划数组dp，其中dp[i]表示第i天的最大利润。
3. 初始化最低价格minPrice为第一天的股票价格prices[0]。
4. 从第二天开始遍历数组，计算当天的最大利润。
5. 对于第i天，更新最低价格minPrice为当前价格prices[i]和minPrice的较小值，以确保在第i天之前买入股票。
6. 当天的最大利润dp[i]等于前一天的最大利润dp[i-1]和当前价格与最低价格之差的较大值，表示在第i天卖出股票的最大利润。
7. 最后返回动态规划数组dp的最后一个元素dp[n-1]，即为可以获得的最大利润。

**时间复杂度分析**

- 遍历数组一次，时间复杂度为O($n$)，其中n为数组长度。

**空间复杂度分析**

- 创建了一个长度为n的动态规划数组，空间复杂度为O($n$)。

### 122. 买卖股票的最佳时机II

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/stock/Solution122.java)

**代码说明**

- 通过遍历价格数组，利用两个状态变量 `dp[0]` 和 `dp[1]` 分别表示持有股票和卖出股票时的最大利润。

- 在每一天，更新持有股票时的最大利润 `dp[0]`，取前一天持有股票的最大利润和前一天卖出股票的最大利润减去当天股票价格的较大值。

- 同样地，也更新卖出股票时的最大利润 `dp[1]`，取前一天卖出股票的最大利润和前一天持有股票的最大利润加上当天股票价格的较大值。

- 最后，返回最后一天卖出股票的最大利润 `dp[1]`，即为能够获得的最大利润。

**时间复杂度**

- O($n$)，其中 n 是数组 prices 的长度。需要遍历整个数组来更新状态。

**空间复杂度**

- O($1$)，使用了大小为 2 的数组 dp 来存储当前持有和卖出股票时的最大利润，空间复杂度是常数级别。

### 123. 买卖股票的最佳时机III

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/stock/Solution123.java)

**代码说明**

- 首先判断数组长度是否小于2，若是则无法完成交易，直接返回0。
- 创建一个二维数组`dp`，其中`dp[i][j]`表示第i天进行第j次交易时的最大利润。
- 初始化第一天的交易情况：
	- `dp[0][1]`表示第一次买入股票的最大利润，初始化为`-prices[0]`。
	- `dp[0][3]`表示第二次买入股票的最大利润，初始化为`-prices[0]`。
- 使用动态规划的思想，从第二天开始遍历数组，根据状态转移方程更新`dp`数组的值。
- 最后返回最后一天完成两次交易的最大利润，即`dp[n - 1][4]`。

**时间复杂度**

- 该算法的时间复杂度为O($n$)，其中n为股票价格数组的长度。

**空间复杂度**

- 该算法使用了一个二维数组`dp`，其大小为n*5，因此空间复杂度为O($n$)。

### 188.买卖股票的最佳时机 IV

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/stock/Solution188.java)

**代码说明**

1. 首先，判断特殊情况：如果数组长度为1或k为0，则无法进行交易，返回0。如果k大于数组长度的一半，相当于可以进行无限次交易，可以直接使用贪心算法解决。
2. 使用三维数组 `dp` 存储状态，其中 `dp[i][j][0]` 表示在第 `i` 天，进行了 `j` 次交易，手上没有持有股票时的最大利润，`dp[i][j][1]` 表示在第 `i` 天，进行了 `j` 次交易，手上持有股票时的最大利润。
3. 初始化状态：第一天没有持有股票时的利润为0，第一天持有股票时的利润为负的买入价格。
4. 使用动态规划的思想，从第二天开始，根据状态转移方程计算每一天的最大利润。
5. 最终结果为 `dp[n-1][k][0]`，表示最后一天完成了 `k` 次交易且手上没有持有股票时的最大利润。

**时间复杂度分析**

- 代码中使用了两层嵌套循环，所以时间复杂度为 O($n*k$)，其中 n 是数组 `prices` 的长度，k 是交易次数。

**空间复杂度分析**

- 代码中使用了一个三维数组 `dp`，大小为 $n(k+1)2$，所以空间复杂度为 O($n*k$)。

### 309.最佳买卖股票时机含冷冻期

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/stock/Solution309.java)

**代码说明**

1. 首先初始化三个数组，分别表示每一天结束时的三种可能状态：持有股票、不持有股票且处于冷冻期、不持有股票且不处于冷冻期。
2. 对于每一天，更新这三种状态的最大利润。这里的更新操作都基于前一天的状态。
3. 在更新操作中，使用了一些max操作来确保始终保留最大利润。
4. 最后，返回最后一天可能的最大利润，这个利润应该是在冷冻期状态和不在冷冻期状态的最大值。

**时间复杂度**

- O($n$)，其中n是prices的长度。需要遍历整个prices数组来更新的动态规划表。

**空间复杂度**

- O($n$)，其中n是prices的长度。使用了三个长度为n的数组来跟踪每一天结束时的三种可能状态的最大利润。

### 714.买卖股票的最佳时机含手续费

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/stock/Solution714.java)

**代码说明**

1. 首先初始化两个数组，分别表示每一天结束时的两种可能状态：持有股票、不持有股票。
2. 对于每一天，更新这两种状态的最大利润。这里的更新操作都基于前一天的状态。
3. 在更新操作中，使用了一些max操作来确保始终保留最大利润。对于不持有股票状态，需要考虑手续费的影响。
4. 最后，返回最后一天可能的最大利润，这个利润应该是不持有股票状态的利润。
...
- O($n$)，其中n是prices的长度。使用了两个长度为n的数组来跟踪每一天结束时的两种可能状态的最大利润。

## 子序列

### 300. 最长递增子序列

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution300.java)

**代码说明**

- 首先，初始化 dp 数组，并将 dp[0] 设置为 1，因为一个元素自身就可以构成一个长度为 1 的递增子序列。

- 接下来，从第二个元素开始，遍历整个数组。对于每个位置 i，查看所有位置在 i 之前的元素，如果有任何一个元素比位置 i 的元素小，那么就更新 dp[i]，使其等于在这些较小元素位置的 dp 值中的最大值 +1。

- 最后，在 dp 数组中找到最大的值，即为最长的递增子序列的长度。

**时间复杂度**

- 时间复杂度为 O($n^2$)，这是因为需要遍历整个数组，对于数组中的每个元素，都需要查看其之前的所有元素。

**空间复杂度**

- 空间复杂度为 O($n$)，使用了一个长度等于数组长度的 dp 数组。

### 674. 最长连续递增序列

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution674.java)

**代码说明**

- 首先，初始化两个变量 `maxLen` 和 `curLen`都为 1。`maxLen`用于保存当前找到的最长连续递增序列的长度，`curLen` 用于保存正在查看的连续递增序列的长度。

- 然后，从第二个元素开始遍历整个数组。对于每个位置 `i`，如果它比前一个位置的元素大，那么就将 `curLen` 增加 1。否则，比较 `maxLen` 和 `curLen`，取较大值为 `maxLen`，并将 `curLen` 重置为 1。

- 最后，返回 `maxLen` 和 `curLen` 中的较大值，这是因为在循环结束后，`curLen` 可能比 `maxLen` 大。

**时间复杂度**

- 时间复杂度为 O($n$)，这是因为只遍历了一次数组。

**空间复杂度**

- 空间复杂度为 O($1$)，只使用了两个额外的变量来保存当前的连续递增序列长度和最长的连续递增序列长度。

### 718. 最长重复子数组

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution718.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示以 `nums1[i-1]` 和 `nums2[j-1]` 为结尾的最长公共子数组的长度。
2. 初始化状态为0，表示没有公共子数组。
3. 使用动态规划的思想，从第一个元素开始，比较两个数组当前位置的元素是否相等，如果相等，则在之前的最长公共子数组长度基础上加1；如果不相等，则最长公共子数组的长度为0。
4. 在遍历过程中记录最长公共子数组的长度，并不断更新。

**时间复杂度分析**

- 代码中使用了两层嵌套循环，所以时间复杂度为 O($m * n$)，其中 m 和 n 分别为数组 `nums1` 和 `nums2` 的长度。

**空间复杂度分析**

- 代码中使用了一个二维数组 `dp`，大小为 $(m+1) * (n+1)$，所以空间复杂度为 O($m * n$)。

### 1143. 最长公共子序列

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution1143.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示text1的前 `i` 个字符与text2的前 `j` 个字符的最长公共子序列的长度。
2. 使用动态规划的思想，从第一个字符开始，比较两个字符串当前位置的字符是否相等。如果相等，则在之前的最长公共子序列长度基础上加1；如果不相等，则取前一个状态中的最大值。
3. 在遍历过程中更新最长公共子序列的长度。

**时间复杂度**

- 代码中使用了两层嵌套循环，所以时间复杂度为 O($m * n$)，其中 m 和 n 分别为字符串 `text1` 和 `text2` 的长度。
...
[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution1035.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示nums1的前 `i` 个数字与nums2的前 `j` 个数字的最大连线数。
2. 使用动态规划的思想，从第一个数字开始，比较两个数组当前位置的数字是否相等。如果相等，则在之前的最大连线数基础上加1；如果不相等，则取前一个状态中的最大值。
3. 在遍历过程中更新最大连线数。
...
[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution53.java)

**代码说明**

1. 使用一维数组 `dp` 存储状态，其中 `dp[i]` 表示以第 `i` 个元素结尾的最大子数组和。
2. 初始化状态：第一个元素自身是一个子数组，所以初始最大子数组和为第一个元素。
3. 使用动态规划的思想，从第二个元素开始，比较包含当前元素的子数组和与重新开始一个新的子数组的和，取其中的最大值作为当前位置的最大子数组和。
4. 在遍历过程中记录最大子数组和。

**时间复杂度**

- 代码中使用了一次遍历，所以时间复杂度为 O($n$)，其中 n 是数组 `nums` 的长度。

**空间复杂度**

- 代码中使用了一个一维数组 `dp`，大小为 n，所以空间复杂度为 O($n$)。

### 392. 判断子序列

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution392.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示字符串 `s` 的前 `i` 个字符是否为字符串 `t` 的前 `j` 个字符的子序列。
2. 初始化状态：空字符串是任何字符串的子序列，所以将 `dp[0][i]` 设为 `true`。
3. 使用动态规划的思想，从第一个字符开始，比较 `s.charAt(i - 1)` 和 `t.charAt(j - 1)` 是否相等，如果相等，则取前一个状态的值；如果不相等，则取当前状态的前一个状态的值。
4. 最终判断 `dp[m][n]` 是否为 `true`，即判断整个字符串 `s` 是否为 `t` 的子序列。

**时间复杂度**

- 代码中使用了两层嵌套循环，所以时间复杂度为 O($m * n$)，其中 m 和 n 分别为字符串 `s` 和 `t` 的长度。
...
1. 使用 HashMap 来构建字符在 T 中的位置索引，键为字符，值为该字符在 T 中出现的位置列表。
2. 遍历输入的字符串 S，对于每个字符 ch，使用二分查找在索引中找到大于等于 prevIndex+1 的最小位置。
3. 如果找到了下一个匹配的位置，则将其作为 prevIndex，并继续处理下一个字符；如果找不到下一个匹配的位置，则 S 不是 T 的子序列。
4. 对于每个输入 S，只需在索引中进行查询，避免了完整遍历和匹配。

这种优化方法可以显著提高算法的效率，使其能够处理大量输入并在较短时间内得出结果。

### 115. 不同的子序列

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution115.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示字符串 `s` 的前 `i` 个字符中字符串 `t` 的前 `j` 个字符出现的次数。
2. 初始化状态：当 `t` 为空字符串时，它是任何字符串的子序列，所以出现次数为1，将 `dp[i][0]` 设为1。
3. 使用动态规划的思想，从第一个字符开始，比较 `s.charAt(i - 1)` 和 `t.charAt(j - 1)` 是否相等，如果相等，则出现次数为 `dp[i - 1][j - 1] + dp[i - 1][j]`；如果不相等，则出现次数为 `dp[i - 1][j]`。
4. 最终返回 `dp[m][n]`，即统计了在字符串 `s` 的子序列中字符串 `t` 出现的次数。

> 注意：由于题目数据保证答案符合32位带符号整数范围，所以将结果的数据类型设置为 `long`，最后转换为 `int` 返回。
...
[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/mindistance/Solution583.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示将 `word1` 的前 `i` 个字符转换为 `word2` 的前 `j` 个字符所需的最小步数。
2. 初始化状态：当一个字符串为空字符串时，将另一个字符串的所有字符都删除，所以 `dp[i][0]` 和 `dp[0][j]` 分别设为 `i` 和 `j`。
3. 使用动态规划的思想，从第一个字符开始，比较 `word1.charAt(i - 1)` 和 `word2.charAt(j - 1)` 是否相等，如果相等，则不需要进行删除操作；如果不相等，则选择删除操作的最小步数。
4. 最终返回 `dp[m][n]`，即将 `word1` 和 `word2` 变为相同所需的最小步数。

**时间复杂度**

- 代码中使用了两层嵌套循环，所以时间复杂度为 O($m * n$)，其中 m 和 n 分别为字符串 `word1` 和 `word2` 的长度。
...
[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/mindistance/Solution72.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示将 `word1` 的前 `i` 个字符转换为 `word2` 的前 `j` 个字符所需的最小操作数。
2. 初始化状态：当一个字符串为空字符串时，需要进行插入或删除操作，所以 `dp[i][0]` 和 `dp[0][j]` 分别设为 `i` 和 `j`。
3. 使用动态规划的思想，从第一个字符开始，比较 `word1.charAt(i - 1)` 和 `word2.charAt(j - 1)` 是否相等，如果相等，则不需要进行操作；如果不相等，则选择插入、删除或替换操作的最小操作数。
4. 最终返回 `dp[m][n]`，即将 `word1` 转换为 `word2` 所使用的最小操作数。
...
[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution647.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示字符串 `s` 的子串从索引 `i` 到索引 `j` 是否为回文串。
2. 初始化时，将长度为1的子串都视为回文串，即 `dp[i][i] = true`。
3. 使用动态规划的思想，从子串长度为2开始，逐步增加子串长度。判断首尾字符是否相等，并根据子串长度和内部子串的回文性质来判断整个子串是否为回文串。
4. 如果当前子串为回文串，则增加回文子串的计数。
5. 最终返回回文子串的数目。

**时间复杂度**

- 代码中使用了两层嵌套循环，所以时间复杂度为 O($n^2$)，其中 n 是字符串 `s` 的长度。

**空间复杂度**

- 代码中使用了一个二维数组 `dp`，大小为 n×n，所以空间复杂度为 O($n^2$)。

### 516. 最长回文子序列

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/09-DynamicProgramming/src/main/java/com/xin/subsequence/Solution516.java)

**代码说明**

1. 使用二维数组 `dp` 存储状态，其中 `dp[i][j]` 表示字符串 `s` 的子串从索引 `i` 到索引 `j` 的最长回文子序列长度。
2. 初始化时，单个字符是回文子序列，长度为1，即 `dp[i][i] = 1`。
3. 使用动态规划的思想，从子序列长度为2开始，逐步增加子序列长度。根据首尾字符是否相等，选择相应的策略来更新最长回文子序列的长度。
4. 如果首尾字符相等，则最长回文子序列的长度为内部子序列长度加2；如果首尾字符不相等，则最长回文子序列的长度为两个子序列中较长的长度。
5. 最终返回 `dp[0][n-1]`，即整个字符串的最长回文子序列长度。
...
[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/10-MonotonicStack/src/main/java/com/xin/Solution739.java)

**代码说明**

1. 使用单调递减栈来解决问题。栈中存储的是温度数组的索引。
2. 遍历温度数组，对于每个元素：
	- 如果栈不为空，并且当前温度大于栈顶索引对应的温度，则说明找到了栈顶索引对应的下一个更高温度的位置，计算天数并更新answer数组。
	- 将当前索引入栈。
3. 返回answer数组作为结果。

**时间复杂度**

- 遍历一次温度数组，每个索引最多入栈一次，出栈一次，因此时间复杂度为O($n$)，其中n为温度数组的长度。

**空间复杂度**

- 使用了一个栈来存储索引，最坏情况下，栈的大小为温度数组的长度n，因此空间复杂度为O($n$)。

## 496.下一个更大元素I

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/10-MonotonicStack/src/main/java/com/xin/Solution496.java)

**代码说明**

1. 使用单调递减栈和哈希表来解决问题。
2. 遍历nums2数组，对于每个元素：
	- 如果栈不为空，并且当前元素大于栈顶元素，则说明找到了栈顶元素的下一个更大元素，将其存储在哈希表中。
	- 将当前元素入栈。
3. 遍历nums1数组，根据哈希表查找每个元素的下一个更大元素并存储在ans数组中。
4. 返回ans数组作为结果。

**时间复杂度**

- 遍历一次nums2数组，每个元素最多入栈一次，出栈一次，并且哈希表的查询操作为O(1)。因此，时间复杂度为O(n)，其中n为nums2数组的长度。

**空间复杂度**

- 使用了一个栈和一个哈希表，栈的最大大小为nums2数组的长度，哈希表的大小取决于nums2中每个元素的下一个更大元素的个数，最坏情况下为nums2数组的长度。因此，空间复杂度为O(n)。

## 503. 下一个更大元素 II

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/10-MonotonicStack/src/main/java/com/xin/Solution503.java)

**代码说明**

1. 使用单调递减栈解决问题。
2. 初始化答案数组ans，将所有元素初始值设为-1。
3. 遍历循环数组nums的两倍长度，对于每个元素：
	- 如果栈不为空，并且当前元素大于栈顶索引对应的元素，则说明找到了栈顶索引对应元素的下一个更大元素，将其存储在答案数组ans中。
	- 如果当前遍历的索引小于原数组长度n，将当前索引入栈。
4. 返回答案数组ans作为结果。

**时间复杂度**

- 遍历循环数组nums的两倍长度，每个元素最多入栈一次，出栈一次。因此，时间复杂度为O(n)，其中n为nums数组的长度。

**空间复杂度**

- 使用了一个栈和一个答案数组，栈的最大大小为原数组的长度n，答案数组的大小为nums数组的长度。因此，空间复杂度为O(n)。

## 42. 接雨水

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/10-MonotonicStack/src/main/java/com/xin/Solution42.java)

**代码说明**

1. 使用单调递减栈解决问题。
2. 遍历柱子高度数组，对于每个柱子：
	- 如果栈不为空，并且当前柱子高度大于栈顶柱子高度，说明形成了一个凹槽可以接雨水。
		- 弹出栈顶柱子的索引，表示凹槽的右边界。
		- 如果栈为空，无法形成凹槽，跳出内循环。
		- 计算凹槽的宽度、高度，并累加接雨水的量。
	- 将当前柱子索引入栈。
...
- 遍历一次柱子高度数组，每个柱子最多入栈一次，出栈一次。因此，时间复杂度为O(n)，其中n为柱子高度数组的长度。

**空间复杂度**

- 使用了一个栈来存储柱子的索引，最坏情况下，栈的大小为柱子高度数组的长度n。因此，空间复杂度为O(n)。

## 84. 柱状图中最大的矩形

[代码链接](https://github.com/HappyAxin/Algorithm/blob/main/Algorithm/10-MonotonicStack/src/main/java/com/xin/Solution84.java)

**代码说明**

1. 使用单调递增栈解决问题。
2. 遍历柱状图高度数组，对于每个柱子高度：
	- 如果栈不为空，并且当前柱子高度小于栈顶柱子高度，说明形成了一个矩形，计算矩形面积。
		- 弹出栈顶柱子的索引，表示矩形的右边界。
		- 如果栈为空，表示矩形左边界为0。
		- 计算矩形的宽度、高度，并更新最大矩形面积。
	- 将当前柱子索引入栈。
3. 返回最大矩形面积。

**时间复杂度**

- 遍历一次柱状图高度数组，每个柱子最多入栈一次，出栈一次。因此，时间复杂度为O(n)，其中n为柱状图高度数组的长度。

**空间复杂度**

- 使用了一个栈来存储柱子的索引，最坏情况下，栈的大小为柱状图高度数组的长度n。因此，空间复杂度为O(n)。
